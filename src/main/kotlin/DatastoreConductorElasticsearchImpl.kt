...
class DatastoreConductorElasticsearchImpl(
    ...
    companion object {
        ...
        private val logger = LoggerFactory.getLogger(DatastoreConductorElasticsearchImpl::class.java)
        init {
    ...
    private var factory: ElasticsearchTransportClientFactory? = null
    ...
    private fun getSimpleSearchQuery(
        ...
        val fuzzy = constraints.fuzzy.get()
        val formattedSearchTerm = if (fuzzy) getFormattedFuzzyString(searchTerm) else searchTerm
        return QueryBuilders.queryStringQuery(formattedSearchTerm)
                .fields(authorizedFieldsMap.values.stream()
                        .flatMap { fieldsMap: Map<String, Float> -> fieldsMap.entries.stream() }
                        .collect(Collectors.toMap(Function<Map.Entry<String, Float>, String> { java.util.Map.Entry.key }, Function<Map.Entry<String, Float>, Float> { java.util.Map.Entry.value })))
                .lenient(true)
    }
    ...
    override fun executeOrganizationSearch(
        ...
        query.filter(QueryBuilders.idsQuery()
                .addIds(*authorizedOrganizationIds.stream().map { aclKey: AclKey -> aclKey[0].toString() }
    ...
    override fun executeEntitySetMetadataSearch(
        ...
        query.filter(QueryBuilders.idsQuery()
                .addIds(*authorizedAclKeys.stream().map { aclKey: AclKey -> aclKey[0].toString() }
                        .toArray { _Dummy_.__Array__() }))
    ...
    override fun triggerOrganizationIndex(organizations: List<Organization>): Boolean {
        ...
        val organizationObjects = organizations.stream()
                .map(Function<Organization, Map<String, Any>> { obj: Organization -> obj.getOrganizationObject() })
                .collect(Collectors.toList())
    ...
    fun triggerIndex(
        ...
        objects.forEach { `object`: Any ->
            try {
                val id = idFn.apply(`object`)
                val s = ObjectMappers.getJsonMapper().writeValueAsString(`object`)
                bulkRequest
